...
const TextEditor=({value,OnChangeEditor}:Props)=>{
        ...
        input.onchange = function () {
          ...
          if (file) {
            ...
            reader.onload = function () {
              ...
              window.tinymce.activeEditor.insertContent(`<img src="${blobInfo.blobUri()}" alt="${file.name}" />`);
            };
      ...
      const videoHandler = () => {
        ...
        input.onchange = function () {
          ...
          if (file) {
            const reader = new FileReader();
            reader.onload = function () {
              window.tinymce.activeEditor.insertContent(`<video controls src="${reader.result}" title="${file.name}"></video>`);
            };
            reader.readAsDataURL(file);
          }
        };
        ...
        input.onchange = async () => {
          ...
          if (file) {
            ...
            reader.onload = function () {
              ...
              window.tinymce.activeEditor?.insertContent(`<audio controls src="${audioSrc}" title="${file.name}"></audio>`);
            };
            reader.readAsDataURL(file);
          }
        };
    ...
    file_picker_callback: (callback, value, meta) => {
      ...
      value={value}
      onChange={(content: string) => OnChangeEditor(content)}
