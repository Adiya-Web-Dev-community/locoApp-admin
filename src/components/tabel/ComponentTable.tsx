...
const ComponentTable = (
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetDataQuery({ url: pageGetUrl });
  ...
  const currentCompanies = response?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const updateElement = (elem) => {
    navigate(`${pageUpdateNav}/${elem._id}`);
    console.log("under process", elem);
  };
  ...
  const handleConfirmDelete = () => {
    ...
    deletPost({
      url: `${pageDeletUrl}/${isModalOpen.id}`,
    })
      .then((res) => {
        if (res.data.success) {
          toast.dismiss();
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <section
              className={`grid grid-cols-${gridColumn} pb-2 p-2  gap-4   min-w-[900px] font-medium md:font-semibold bg-white font-mavenPro`}
            >
            ...
            <div className=" h-[380px] overflow-y-auto [&::-webkit-scrollbar]:hidden min-w-[900px] bg-gray-50">
              {isLoading ? (
                ...
                response?.map((element, i) => (
                  <section
                    key={i}
                    className={`grid items-center gap-6 py-2 pl-6 pr-4 border-t-2 border-gray-200 grid-cols-${gridColumn} group hover:bg-gray-50`}
                  >
                    <span>{i + 1}</span>
                    <div className="flex items-center justify-center">
                      {element?.thumnail ? (
                        <img
                          ...
                          alt="element Image"
                          className="object-contain w-full rounded-lg h-1/2"
                        />
                      ) : (
                    ...
                    <div className="grid justify-center gap-2">
                      ...
                      <button
                        ...
                        onClick={() => deletElement(element._id)}
                      >
                        Delete
                      </button>
                    </div>
