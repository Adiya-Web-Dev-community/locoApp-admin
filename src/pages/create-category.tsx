  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className=" w-auto flex  mb-4 flex-wrap gap-3  rounded-[5px] ">
          {tablist?.map((item, index) => {
            return (
              <div
                key={index}
                onClick={() => setTab(item.value)}
                className={`relative cursor-pointer py-2 px-4 font-semibold  text-[#242424] transition-all duration-300 ease-in-out transform ${
                  tab === item.value
